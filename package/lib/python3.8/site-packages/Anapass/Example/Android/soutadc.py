...
def print_help() :
    print(" ")
    print("ted.sdoutadc --<option-name>=<options-value> <chIdx0> <chIdx1>... ")
    print(" ")
    print("   <options-name>")
    print("       --dut:  dut index (default : 0)")
    print("       --devconfig :  Average Sample Number")
    print("       --inconfig:  set interval")
    print("       --incfgch :  ch number of inconfig")
    print("       --rbsel :  select rb")
    print(" ")
    ...
    print(" ")
    print("   Example:")
    print("       Ex1) ted.sdoutadc -v")
    print("       Ex1) ted.sdoutadc -v 0 1 10")
    print("       Ex3) ted.ldoadc --dut=1 -v 0 1 10")
    print("       Ex3) ted.sdoutadc --devconfig=0x71E0")
    print("       Ex3) ted.sdoutadc --inconfig=0x740 --inch=3")
    print(" ")
...
def main(argv) :
    ...
    try :
        ...
        opts, etc_args = getopt.getopt(argv[1:], "hd:e:i:c:r:v", ["help", "dut=", "devconfig=", "inconfig=", "incfgch=", "rbsel=", "voltage" ])
    ...
    for opt, arg in opts :
        ...
        if opt in ("-h", "--help") :
        ...
        elif opt in ("-d", "--devconfig") :
            devConfigValue = string_to_int(arg, "--devconfig")
            devConfigValid = True
            ...
            rbSelValue = string_to_int(arg, "--rbsel")
            rbSelValid = True
            if (rbSelValue != 0) and (rbSelValue!= 1)  :
                print("ERROR: The values of rbsel must be  0  or 1")
                sys.exit()
    ...
    if devConfigValid : 
        bRet = ted.sout_adc_set_devconfig(dutIdx, devConfigValue)
        if bRet : resMsg="OK"
        else : resMsg = "Fail"
        print("devconfig : 0x%X ---- %s "%(devConfigValue, resMsg))
    ...
    if inConfigValid :  
        ...
        bRet = ted.sout_adc_set_inconfig(dutIdx, inChValue, inConfigValue)
        if bRet : resMsg="OK"
        else : resMsg = "Fail"
        print("inconfig(ch=%d) : 0x%X ---- %s "%(inChValue, inConfigValue, resMsg))

    if rbSelValid : 
        bRet = ted.sout_adc_set_rbsel(dutIdx, rbSelValue)
        if bRet : resMsg="OK"
        else : resMsg = "Fail"
        print("rbsel : %d ---- %s "%(rbSelValue, resMsg))
    ...
    if( len(etc_args) == 0) : 
        if displayVoltage :
            powerVoltage = ted.sout_adc_get_all_voltage(dutIdx)
        ...
        if displayVoltage :
            for chIdx in range(channelMaxCount) :
                if channelValidList[chIdx] :
                    powerVoltage[chIdx] = ted.sout_adc_get_voltage(dutIdx, chIdx)
    
    if displayVoltage :
        channelCount = ted.sout_adc_get_channel_count(dutIdx)
        print("SoutADC, dutIdx=%d dutCnt=%d Ch.Cnt=%d " % (dutIdx, dutCount, channelCount) )    
        if channelCount > 0 :
            for chIdx in range(channelMaxCount) :
                if channelValidList[chIdx] :
                    msg = "Ch%d"%chIdx
