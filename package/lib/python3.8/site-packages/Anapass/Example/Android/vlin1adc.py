...
def print_help() :
    print(" ")
    print("ted.vlin1adc --<option-name>=<options-value> <addr> <PowerName> ")
    print(" ")
    print("   <options-name>")
    ...
    print(" ")
    print("   Example:")
    print("       Ex1) ted.vlin1adc -v ")
    print("       Ex2) ted.vlin1adc -a ")
    print("       Ex3) ted.regadc --dut=1 -v ")
    print("       Ex4) ted.vlin1adc --samples=4 -v  VLIN1 VBAT")
    print("       Ex5) ted.vlin1adc --samples=4 -v -a VBAT")
    print(" ")
...
def main(argv) :
    ...
    powerNameTuple=("VLIN1", "VBAT", "ELVDD")
    ...
    if samplesValue >= 0 : 
        bRet = ted.vlin1_adc_set_samples(dutIdx, samplesValue)
        if bRet : resMsg="OK"
        else : resMsg = "Fail"
        print("samples : %d ---- %s "%(samplesValue, resMsg))

    if intervalValue >= 0 :  
        bRet = ted.vlin1_adc_set_interval(dutIdx, intervalValue)
        if bRet : resMsg="OK"
        else : resMsg = "Fail"
        print("interval : %d ---- %s "%(intervalValue, resMsg))

    if channel_on_Value >= 0 :  
        bRet = ted.vlin1_adc_set_channel_on(dutIdx, channel_on_Value)
        if bRet : resMsg="OK"
        else : resMsg = "Fail"
        print("channel_on : %d ---- %s "%(channel_on_Value, resMsg))

    if channel_off_Value >= 0 :  
        bRet = ted.vlin1_adc_set_channel_off(dutIdx, channel_off_Value)
        if bRet : resMsg="OK"
        else : resMsg = "Fail"
        print("channel_off : %d ---- %s "%(channel_off_Value, resMsg))

    if modeValue >= 0 :  
        bRet = ted.vlin1_adc_set_mode(dutIdx, modeValue)
        if bRet : resMsg="OK"
        else : resMsg = "Fail"
    ...
    if displayVoltage :
        for powerIdx in range(powerCount) :
            if powerValidList[powerIdx] :
                powerVoltage[powerIdx] = ted.vlin1_adc_get_voltage(dutIdx, powerIdx)
    
    if displayCurrent :
        for powerIdx in range(powerCount) :
            if powerValidList[powerIdx] :
                powerCurrent[powerIdx] = ted.vlin1_adc_get_current(dutIdx, powerIdx)
    
    if displayVoltage | displayCurrent :
        print()
        print("-------------------------------------------------");
        print("Vlin1ADC, dutIdx=%d dutCnt=%d " % (dutIdx, dutCount) )    
        print("-------------------------------------------------");
        for powerIdx in range(powerCount) :
            if powerValidList[powerIdx] :
