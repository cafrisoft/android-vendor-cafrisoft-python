                  ...
                  wchar_t *prefix, size_t prefix_len, int *found)
{
    ...
    if (argv0_path != NULL) {
        PYMYDBGMSG("argv0_path=%s", Py_EncodeLocale(argv0_path, NULL));
    }
    else {
        PYMYDBGMSG("argv0_path is NULL");
    }
    ...
    if (pathconfig->home) {
        ...
        PYMYDBGMSG("pathconfig->home=%s", Py_EncodeLocale(pathconfig->home, NULL));
        ...
        *found = 1;
        ...
        PYMYDBGMSG("pathconfig->home is NULL");
    }
    ...
    PYMYDBGMSG("path=[%s] ", Py_EncodeLocale(path, NULL));
    ...
    if (isfile(path)) {
        ...
        PYMYDBGMSG("isfile(path) is TRUE");
        ...
        PYMYDBGMSG("isfile(path) is FALSE");
    }
                 ...
                 wchar_t *prefix, size_t prefix_len)
{
    ...
    if (!calculate->prefix_found) {
        ...
        if (calculate->warnings) {
            fprintf(stderr,
                "Could not find platform independent libraries <prefix> HHT \n");
        }
...
#ifdef ANDROID
    wcscat(buf, delimiter);
    wcscat(buf, ANAPASS_TESYSDATA_PYTHON_WPATH);
#endif
...
calculate_init(PyCalculatePath *calculate, const PyConfig *config)
{
    ...
    if (path) {
        ...
        if (!calculate->path_env) {
            ...
            PYMYDBGMSG("[hthwang] calculate->path_env :  [%s] ", Py_EncodeLocale(calculate->path_env, NULL) );
        }
    }
    else {
        fprintf(stderr, "[hthwang] path is NULL \n");
    }
    ...
    if (!calculate->pythonpath) {
        ...
        PYMYDBGMSG("[hthwang] calculate->pythonpath :  [%s] ", Py_EncodeLocale(calculate->pythonpath, NULL) );
    }
    ...
    if (!calculate->prefix) {
        ...
        PYMYDBGMSG("[hthwang] calculate->prefix :  [%s] ", Py_EncodeLocale(calculate->prefix, NULL));
    }
    ...
    if (!calculate->exec_prefix) {
        ...
        PYMYDBGMSG("[hthwang] calculate->exec_prefix :  [%s] ", Py_EncodeLocale(calculate->exec_prefix, NULL) );
    }
    ...
    if (!calculate->lib_python) {
        ...
        PYMYDBGMSG("[hthwang] calculate->lib_python :  [%s] \n", Py_EncodeLocale(calculate->lib_python, NULL));
    }
...
calculate_path(PyCalculatePath *calculate, _PyPathConfig *pathconfig)
{
    ...
    PYMYDBGMSG("calculate->exec_prefix:[%s] ", calculate->exec_prefix);
    ...
    PYMYDBGMSG("exec_prefix: [%s] calculate->exec_prefix:[%s] pathconfig->exec_prefix : [%s] ", 
        Py_EncodeLocale(exec_prefix, NULL),
        Py_EncodeLocale(calculate->exec_prefix, NULL),
    ...
    PYMYDBGMSG("exec_prefix: [%s] calculate->exec_prefix:[%s]  ",
        Py_EncodeLocale(exec_prefix, NULL),
        Py_EncodeLocale(calculate->exec_prefix, NULL)
    );
    ...
    PYMYDBGMSG("pathconfig->module_search_path: [%s] calculate->exec_prefix:[%s]  ",
        Py_EncodeLocale(pathconfig->module_search_path, NULL),
        Py_EncodeLocale(calculate->exec_prefix, NULL)
    );
    ...
    if (pathconfig->exec_prefix == NULL) {
        ...
        PYMYDBGMSG("exec_prefix: [%s] calculate->exec_prefix:[%s] pathconfig->exec_prefix : [%s] ",
            Py_EncodeLocale(exec_prefix, NULL),
            Py_EncodeLocale(calculate->exec_prefix, NULL),
            Py_EncodeLocale(pathconfig->exec_prefix, NULL) );
